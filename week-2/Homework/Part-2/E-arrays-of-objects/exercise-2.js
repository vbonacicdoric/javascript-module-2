...
let destination1 = {
  ...
  */
  ...
  let destinationNamesWithin500Kms = travelDestinations
    .filter((element) => element.distanceKms <= 500)
    .map((element) => element.destinationName); // Complete here
  ...
  let destinationNamesMoreThan300KmsAwayByTrain = travelDestinations
    .filter(
      (element) =>
        element.distanceKms > 500 && element.transportations.includes("train")
    )
    .map((element) => element.destinationName); // Complete here (PRINT THE RESULT IN THE CONSOLE USING FOREACH)
  /*
